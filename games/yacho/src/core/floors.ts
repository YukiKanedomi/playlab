...
// クセ敵/エリート枠へ降格して1層0〜1体に絞る（層9のみ「混成」として2種そろい踏み。夜間監査[D]で3種→2種に減らした）。
...
// 夜間監査[D]2：層構成をなだらかに（3,4,5+岩殻,6+胞子,6+穴潜み,7,8+胞子,8+穴潜み,10+クセ敵2種,ボス+4）。
// 旧構成は1層6体・2層8体と入口から敵アイコンだらけで「本当の3マッチを学ぶ」原則に反していたため、
// 1〜2層は素のサンドバッグだけを少数にして、以降なだらかに積み増す形へ差し替えた。
export const FLOORS: FloorDef[] = [
  { floor: 1, enemies: swarm(3), env: null }, // 本当の3マッチを学ばせる入口（監査[D]推奨）
  { floor: 2, enemies: swarm(4), env: null },
  { floor: 3, enemies: [...swarm(5), { kind: 'rockshell', at: { x: 4, y: 4 } }], env: null },
  { floor: 4, enemies: [...swarm(6), { kind: 'sporeling', at: { x: 4, y: 4 } }], env: null },
  { floor: 5, enemies: [...swarm(6), { kind: 'burrower', at: { x: 4, y: 4 } }], env: 'fungal' },
  { floor: 6, enemies: swarm(7), env: null },
  { floor: 7, enemies: [...swarm(8), { kind: 'sporeling', at: { x: 4, y: 4 } }], env: null },
  { floor: 8, enemies: [...swarm(8), { kind: 'burrower', at: { x: 4, y: 4 } }], env: 'crystal' },
  {
    floor: 9,
    // 混成：クセ敵2種（岩殻獣+穴潜み）＋サンドバッグ最大数（比率是正の例外＝§4「層9=swarm10+混成」）
    enemies: [...swarm(10), { kind: 'rockshell', at: { x: 4, y: 4 } }, { kind: 'burrower', at: { x: 5, y: 4 } }],
    env: null,
  },
  {
    floor: 10,
