 ...
 *
 * **2026-08-15 オーナー指示で一時停止（Infinity）**：毎回の入れ替え強制と手持ち確認がストレス
 * （「バランスはさておき、どんどん知見が増えていく仕様に戻す」）。入れ替え画面・discardUpgrade 等の
 * 機構は残してあり、この定数を有限に戻すだけで8枠制に復帰できる。頭打ちの実測（PHASE2 §2）は有効なまま。
 */
export const UPGRADE_SLOTS_DEFAULT = Infinity
